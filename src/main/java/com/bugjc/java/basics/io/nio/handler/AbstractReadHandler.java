...
public abstract class AbstractReadHandler {
     ...
     * 存放未处理完成的包
     */
    private LinkedList<MsgPacket> packetQueue = new LinkedList<MsgPacket>();
    ...
    AbstractReadHandler(SocketChannel socketChannel) {
            ...
            if (bytes[0] == MsgPacket.MSG_FLAG) {
                if (bytes.length < MsgPacket.HEADER_SIZE) {
                    ...
                    if (bytes.length - header.length == 0) {
                        ...
                        int length = Math.min(bytes.length, msgPacket.getNeedDataLength());
...
//						packetQueue.add(msgPacket);
     ...
     * @return
     */
    private byte[] adjust(byte[] bytes, int more) {
